...
# Подкачка: обычный файл на карте, zram по умолчанию выключен.
#
# Это результат замеров, а не предпочтение. Проверялось на игре, которой нужно
# около 1,8 ГБ, одинаковыми отрезками по девять минут:
#
...
#   zram 256 МБ lz4 + файл 2 ГБ    - играет; на карту 1,66 ГБ, свободно 143 МБ
#   zram 256 МБ zstd + файл 2 ГБ   - играет; на карту 1,38 ГБ, свободно 177 МБ
#   только файл 2 ГБ (стало)       - играет; на карту 1,41 ГБ, свободно 183 МБ
#
# Главный вывод неочевиден: zram не снижает запись на карту. Он действительно
# принимает на себя свои 268 МБ, но ровно настолько же отнимает памяти у игры,
# и ядру приходится выгружать больше в целом - 1,78 ГБ против 1,42 ГБ без него.
# Итог на карте одинаковый, а чтений обратно без zram меньше (0,84 против 0,97
# ГБ), и на устройстве это ощущается как более плавная игра.
#
# zram остаётся настройкой: под другую нагрузку он может подойти, и алгоритм по
