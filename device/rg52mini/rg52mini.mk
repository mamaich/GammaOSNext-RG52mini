...
# Подкачка: zram на всю память, файла подкачки нет.
#
# Это заводское поведение Android, и выбрано оно осознанно, с известной ценой.
# Замеры на игре, которой нужно около 1,8 ГБ при 2 ГБ памяти, отрезками по
# девять минут:
#
#   zram во всю память, без файла   - игра умирает за ~2 минуты, и убийца по
#                                     дороге сносит лаунчер, microG и магазин;
#                                     969 МБ данных в zram занимают 420 МБ ОЗУ,
#                                     свободными остаются 48 МБ, thrashing 253%
#   zram 256 МБ zstd + файл 2 ГБ    - играет; на карту 1,38 ГБ, свободно 177 МБ
#   только файл 2 ГБ                - играет плавнее всего; на карту 1,41 ГБ
#
# Почему всё же zram. Тяжёлая игра здесь - исключение: обычная нагрузка это
# эмуляторы и небольшие приложения, которым сжатия в памяти хватает с запасом, а
# карта при этом не изнашивается вовсе. Для исключений есть переключатель прямо
# в меню выключателя ("Swap Mode"), который меняет режим без перезагрузки.
#
# Чего этот режим НЕ делает: файл подкачки ему не страховка. Страницы попадают в
# файл только когда zram упёрся в свой потолок, а при потолке во всю память
# этого не случается - убийца срабатывает раньше. Поэтому пара "большой zram +
# файл" ведёт себя как zram без файла (проверено: 512 МБ с файлом игру тоже не
# спасают), и в меню режим "вместе" намеренно берёт небольшой zram.
#
# Алгоритм zstd, а не заводской lz4: на тех же данных он держит 268 МБ в 74 МБ
# памяти против 105 МБ у lz4.
PRODUCT_SYSTEM_DEFAULT_PROPERTIES += \
    persist.rg52.zswap.enabled=0 \
    persist.rg52.zswap.max_pool_percent=20 \
    persist.rg52.zswap.algo=zstd \
    persist.rg52.zram.size_mb=100% \
