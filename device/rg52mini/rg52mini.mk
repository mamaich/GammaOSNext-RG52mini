...
#
# Время удержания оставлено штатное, две секунды
# (persist.gammaos.gamepad.mouse_hold_ms). Учтите разницу с rgp2pad: там аккорд
# ловился по почти одновременному нажатию и срабатывал сразу, а здесь всё
# время отсчёта нажатия проходят в приложение как обычные кнопки, и лишь при
# срабатывании gammapad досылает им синтетические отпускания, чтобы кнопки не
# залипли. То есть игра успевает увидеть двухсекундное нажатие на стики.
PRODUCT_SYSTEM_DEFAULT_PROPERTIES += \
...
#   R3 - правая кнопка
#   A, B, X, Y - обычные кнопки геймпада, как и вне режима мыши
#   L1 - медленный ход курсора, пока удерживается
#
# Стики остаются и аккордом включения режима: удержание обоих секунду. Одиночное
# нажатие при этом работает как кнопка мыши - аккорд ловится отдельно, по двум
# сразу. Цена решения: пока идёт отсчёт секунды, обе кнопки мыши оказываются
# нажатыми, но сразу после этого режим выключается вместе с самой мышью.
#
# mouse_btn_back=0 отключает привязку: код 0 не приходит никогда, поэтому B
# просто уходит в приложение. Кнопка "назад" на корпусе при этом работает как
# обычно, она отдельная.
#
# Медленный ход переехал с X на L1: X понадобился как обычная кнопка.
PRODUCT_SYSTEM_DEFAULT_PROPERTIES += \
    ...
    persist.gammaos.gamepad.mouse_btn_boost=310 \
    persist.gammaos.gamepad.mouse_hold_ms=1000
