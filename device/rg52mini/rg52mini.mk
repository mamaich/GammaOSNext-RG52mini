...
# Сборщик мусора ART с уплотнением (userfaultfd CMC) вместо копирующего.
#
# База phh выставляет здесь false, и по делу: GSI не несёт ядра, проба на
# этапе сборки видит <unknown-kernel> и включила бы CMC, а на устройствах с
# ядром без userfaultfd (всё до 5.7) ART откатился бы на копирующий сборщик с
# барьерами чтения. Это расходится с загрузочным образом, проверка OatHeader
# падает, и zygote пересобирает образ через dex2oat при каждой холодной
# загрузке — на слабом железе это 16-20 секунд. Разбор — в комментарии
# device/phh/treble/base.mk.
#
# У нас ядро своё, и в нём userfaultfd теперь есть: /proc/sys/vm/
# unprivileged_userfaultfd на устройстве появился, CONFIG_USERFAULTFD=y.
# Значит пара «загрузочный образ + среда выполнения» сходится, и мы получаем
# то, ради чего всё затевалось: копирующему сборщику нужен запас памяти под
# копию кучи, уплотняющий работает на месте. На 2 ГБ это заметная статья.
