...
# Размер раздела system намеренно постоянный, а не по содержимому. Обновление по
# воздуху пишет в раздел целую файловую систему: если раздел на карте окажется
# хоть на блок меньше записываемой ФС, ext4 отказывается её монтировать —
#
#     EXT4-fs (mmcblk1p4): bad geometry: block count 768484 exceeds size of
#                          device (763136 blocks)
#
# init не может смонтировать /system, вызывает InitFatalReboot, устройство уходит
# в режим loader, u-boot отказывается прошивать SD и валится в bootrom — петля,
# из которой выводит только перезапись карты. Ровно это и случилось 24.09.2026:
# раздел считался по содержимому, урезанный roms.tar.zst сжал его на 21 МБ, и
# следующее обновление перестало влезать. Постоянный размер означает, что разметка
# у всех сборок одна, и любое обновление в неё войдёт.
SYS_SECTORS=6963200              # 3400 МиБ, менять только вместе с отказом от OTA
SYSNEED=$(( (SYSBYTES + 511) / 512 ))
if [ "$SYSNEED" -gt "$SYS_SECTORS" ]; then
    echo "system.img ($(( SYSBYTES / 1048576 )) МиБ) не влезает в раздел ($(( SYS_SECTORS / 2048 )) МиБ)." >&2
    echo "Увеличивать SYS_SECTORS можно, но обновление по воздуху на карты прежних" >&2
    echo "сборок после этого не встанет — им понадобится перезапись карты." >&2
    exit 1
fi
...
echo "раздел system: $(( SYS_SECTORS / 2048 )) МиБ постоянно, запас $(( (SYS_SECTORS - SYSNEED) / 2048 )) МиБ"
...
# --- выбор драйвера Wi-Fi из своей сборки ---
# Скрипт приехал в vendor от Android 13 и ждал появления интерфейса вслепую -
# до четырёх секунд. На ревизии B, то есть у большинства, эти секунды чистая
# потеря, и не безобидная: прошивка AIC8800 поднимается ещё около девяти секунд
# после insmod, а Android начинает подключаться, как только видит wlan0.
# Задержка сдвигала готовность прошивки ровно в тот момент, когда каркас уже
# пробует связаться, и попытка отбивалась изнутри драйвера с кодом 30, после
# которого Android ждёт пять минут. Столько устройство и оставалось без сети
# после каждой загрузки.
#
# Наш вариант ждёт только если rk915 значится в /proc/modules, то есть если есть
# чего ждать. Разбор - в doc/rg52mini/02-план.md.
WPICK=${WPICK:-$TREE/device/rg52mini/wifi_pick.sh}
if [ -f "$WPICK" ] && [ -f /mnt/imgven/bin/wifi_pick.sh ]; then
    sudo cp -f "$WPICK" /mnt/imgven/bin/wifi_pick.sh
    sudo chmod 755 /mnt/imgven/bin/wifi_pick.sh
    echo "   выбор Wi-Fi: /vendor/bin/wifi_pick.sh из своей сборки ($(stat -c %s "$WPICK") байт)"
else
    [ -f "$WPICK" ] || echo "   !! нет $WPICK — останется скрипт из эталона, с ожиданием вслепую"
fi
