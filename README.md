...
| Дерево исходников развёрнуто | готово, докачивается LFS |
| Отдельный продукт сборки | **не нужен** — стандартная цель даёт нужную разметку |
| Первая сборка `system.img` | в работе |
| Устройство-зависимые добавки в GSI | подготовлены, ждут первой удачной сборки |
